            ...
            @foreach($bookings as $booking)
              ...
              <tr>
                ...
                @php
                  ...
                  @if($expired_date >= date('Y-m-d'))
                    @if($booking->status == 0)
                      <a href="{{ route('status.update',['approved',$booking->id]) }}" class="btn btn-default btn-sm btn-icon icon-left">
                        <i class="entypo-check"></i>
                        Approve
                      </a>
                    @else
                      <a href="{{ route('status.update',['pending',$booking->id]) }}" class="btn btn-default btn-sm btn-icon icon-left">
                        <i class="entypo-check"></i>
                        Pending
                      </a>
                    @endif
                  @endif
                  <a href="#" class="btn btn-danger btn-sm btn-icon icon-left" onclick="confirm_modal('{{ 'bookings' }}','{{ $booking->id }}');">
