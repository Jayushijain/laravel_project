		...
		<div class="row justify-content-center">
			...
			<div class="col-md-4 mb-3">
				<div class="category-title">
					<a href="/listingsview/{{$category['id']}}<?php //echo site_url('home/filter_listings?category='.slugify($category['name']).'&&amenity=&&video=0&&status=all'); ?>" style="color: unset;"><?php echo $category['name']; ?> <small style='font-size: 12px;'>(<?php echo count_sub_category($category['id']); ?>)</small></a>
				</div>
				<?php
				...
				//foreach ($sub_categories->result_array() as $key => $sub_category):
					...
					?>
				<a href="/listingsview/{{$category['id']}}<?php //echo site_url('home/filter_listings?category='.slugify($sub_category['name']).'&&amenity=&&video=0&&status=all'); ?>" class="sub-category-link">
