<?php
    ...
    {
        $settings = DB::table('frontend_settings')->where('type', $type)->value('description');
 ...
 */
 ...
 //    }
    ...
    {
        $reviewer = DB::table('reviews')->where('listing_id', $listing_id)->get();
        return $reviewer; 
    }
...
// $user_details = $CI->db->get_where('user', array('id' => $CI->session->userdata('user_id')))->row_array();
  ...
  {
    $country = DB::table('countries')->where('id', $country_id)->value('name');
    return $country;
  }
 ...
 */
  ...
  if ( ! function_exists('slugify'))
  {
    ...
    {
        $categories = DB::table('categories')->where('id', $categories)->value('name');
        return $categories;
    }

    function get_amenity_name($amenity = "")
    {
        $amenity = DB::table('amenities')->where('id', $amenity)->value('name');
        return $amenity;
    }
...
if (! function_exists('has_package')) {
  function has_package($user_id = 0, $pacakge_details = false) {
    ...
    if ($user_id > 0) {
      $user_details = App\User::where('id',$user_id)->first();
...
function get_user_wise_wishlist() 
{
  ...
  if (count($wishlists) > 0) 
  {
  	...
  	{
  		$user_wishlists = App\Listing::where('id',$wishlists)->get();
  	}
